...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	float interval = 0.3f;
	instances[0].position = XMFLOAT3(0.0f, -1.0f, 0.0f);
	for (int i = 1; i < 300; ++i) {
		float xPos = (rand() % 200 - 100) * interval; // -50ºÎÅÍ 50±îÁö ·£´ýÇÑ x ÁÂÇ¥
		float zPos = (rand() % 200 - 100) * interval; // -50ºÎÅÍ 50±îÁö ·£´ýÇÑ z ÁÂÇ¥
		instances[i].position = XMFLOAT3(xPos, -1.0f, zPos);
	}
...
void ModelClass::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	ID3D11Buffer* bufferPoinsters[2] = { m_vertexBuffer, m_instanceBuffer };

	// Set the vertex buffer to active in the input assembler so it can be rendered.
	deviceContext->IASetVertexBuffers(0, 2, bufferPoinsters, strides, offsets);
