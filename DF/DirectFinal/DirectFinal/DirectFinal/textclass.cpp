...
bool TextClass::SetMousePosition(int mouseX, int mouseY, int faceCount, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char pollygonString[32];
	char mouseString[32];
	...
	_itoa_s(faceCount, tempString, 10);

	strcpy_s(pollygonString, "Pollygon = ");
	strcat_s(pollygonString, tempString);

	result = UpdateSentence(m_sentence3, pollygonString, 20, 60, 1.0f, 1.0f, 0.0f, deviceContext);

	_itoa_s(m_screenWidth, tempString, 10);

	strcpy_s(mouseString, tempString);
	strcat_s(mouseString, " X ");

	_itoa_s(m_screenHeight, tempString, 10);

	strcat_s(mouseString, tempString);

	result = UpdateSentence(m_sentence4, mouseString, 20, 80, 1.0f, 1.0f, 0.0f, deviceContext);

	strcpy_s(mouseString, "State");

	result = UpdateSentence(m_sentence5, mouseString, 20, 100, 1.0f, 0.0f, 0.0f, deviceContext);
